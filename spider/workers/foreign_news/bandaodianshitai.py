...
def get_info():
    ...
    # 打开Google首页
    browser.get('https://www.aljazeera.net/')
    '''
        ID = "id"
        XPATH = "xpath"
        LINK_TEXT = "link text"
    ...
    for i in range(5):
        ...
        id_tag = []
        output_tag = ['#root > div > div:nth-child(2)']
        ...
        for tag in output_tag:
            ...
            for j, result in enumerate(samples):
                ...
                if url is not None and text is not None and 'http' in url and len(text) > 20 and text not in titles:
                    titles.add(text)
                    # print(text)
