...
def get_info():
    ...
    # 打开Google首页
    browser.get('https://www.famitsu.com/ranking/article/')
    '''
        ID = "id"
        XPATH = "xpath"
        LINK_TEXT = "link text"
    ...
    for i in range(1):
        ...
        output_tag = ['body > div.l-main > div.l-story.container > div > div.col-12.col-md-12.col-lg-8 > div.row.border-col-bottom.js-not-transition-change']
        ...
        for tag in output_tag:
                ...
                samples = result.find_elements('css selector', 'div > div > div > div.card__body')
                for j, result in enumerate(samples):
                        content = result.find_element('css selector', 'h2 > a')
                        url = content.get_attribute('href')
                        text = content.text
                        try:
                            desc = result.find_element('css selector', 'p > a').text
                        except:
                            desc = ''
