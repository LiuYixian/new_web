...
def get_info():
    ...
    # 打开Google首页
    browser.get('https://www.gameinformer.com/news')
    '''
        ID = "id"
        XPATH = "xpath"
        LINK_TEXT = "link text"
    ...
    for i in range(1):
        ...
        for tag in output_tag:
            ...
            samples = result.find_elements('css selector', 'div > article > div.teaser-right > div')
            for j, result in enumerate(samples):
                content = result.find_element('css selector', 'h2 > a')
                url = content.get_attribute('href')
                text = content.text
                desc = result.find_element('css selector', 'div.field.field--name-field-promo-summary.field--type-string.field--label-hidden.gi5-field-promo-summary.gi5-string.field__item').text
            ...
            for j, result in enumerate(samples):
                ...
                try:
                    desc = result.find_element('css selector',
                                           'div.field.field--name-field-promo-summary.field--type-string.field--label-hidden.gi5-field-promo-summary.gi5-string.field__item').text
                except:
                    desc = ''
