...
def get_info():
    ...
    # 打开Google首页
    browser.get('https://www.eurogamer.net/archive/news')
    '''
        ID = "id"
        XPATH = "xpath"
        LINK_TEXT = "link text"
    ...
    for i in range(1):
        ...
        for tag in output_tag:
            ...
            samples = result.find_elements('css selector', 'li > div')
            for j, result in enumerate(samples):
                    content = result.find_element('css selector', 'div.details > p.title > a')
                    url = content.get_attribute('href')
                    text = content.text
                    try:
                        desc = result.find_element('css selector', 'div.details > p.strapline').text
                    except:
                        desc = ''
